...
Public Class Login
    ...
    Private Sub btnLogin_Click(sender As System.Object, e As System.Windows.RoutedEventArgs) Handles btnLogin.Click
        ...
        Dim da As New SqlCeDataAdapter("SELECT * FROM CATC WHERE (EmpNum='" & tbEmployeeNum.Text & "')", con)
        Dim cb As New SqlCeCommandBuilder(da)
        If tbEmployeeNum.Text <> "Employee Number" And tbPassword.Password <> "Password" Then
            command.CommandText = "SELECT COALESCE(COUNT(*),0) FROM CATC WHERE (EmpNum='" & tbEmployeeNum.Text & "' AND Type= 'Counselor')"
            con.Open()
            ...
            If aVerify = 1 Then
                ...
                da.Fill(ds, "CATC")
                command.CommandText = "SELECT Department.Name FROM CATC INNER JOIN Department ON CATC.Handles=Department.DeptCode WHERE EmpNum='" & tbEmployeeNum.Text & "'"
                Dim affiliation As String = command.ExecuteScalar().ToString()
                con.Close()
                StringPassCheck = ds.Tables("CATC").Rows(0).Item("Password").ToString()
                ...
                If tbPassword.Password.Equals(StringPassCheck) Then
                    ...
                    ActiveDeptCode = ds.Tables(0).Rows(0).Item("Handles").ToString
                    Try
